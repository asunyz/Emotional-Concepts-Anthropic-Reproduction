...
### 07:30 UTC — L30 surprise_affective inspection + data-design diagnosis

- **L30 feat 92068 (surprise_affective candidate #1)**: Level-0 sanity reveals it's a broad general-emotion feature, not surprise-specific. nz_rate 92.7% on surprise / 61.7% overall, top-50 stories near-uniform across 9 concepts (surprise=9, angry=9, afraid=8, inspired=6, awestruck=6...). Specificity 0.746 was a small-numerator/small-denominator artifact. **Drop.**
- **L30 feat 46443 (surprise_affective candidate #2)**: Level-0 looks promising — top-50 = surprise(23) + awestruck(17) + others(10). Manual peak-token inspection on top-10 stories shows a remarkably consistent semantic pattern in the ±6 token windows around argmax:
  - "needing to **confirm** they were still solid"
  - "the **world** looked sharper, more vivid"
  - "remind **herself** that she was still here"
  - "**forgot** how to breathe, suspended in..."
  - "the **world** was blurry, not from tears"
  - "the pain was real"
  - "sense of unreality that made the world tilt"
  - "trying to ground himself in the present"
  - "unable to **process** how something so ordinary..."
  These are all **post-impact reality-recalibration / mild-derealization scenes** — characters checking whether reality is still real after a strong event. nz_tokens covers 25–50% of each top story, so it's a paragraph-level state feature, not a single-token trigger. Activates on surprise+awestruck (positive-valence high-arousal cluster) and not on afraid (negative-valence — characters in fight/flight, not recalibration).
- **Mechanism implication**: this is the kind of axis SAE adds *over* mean-diff. Mean-diff(surprise − neutral) couldn't isolate it because: (a) it crosses surprise+awestruck+occasional sad, not aligned with any concept label; (b) it's a narrative-state dimension (reality-recalibration phase), not a lexical/affect dimension; (c) it's distributed across many tokens, which story-level mean averaging dilutes.
- **HOWEVER — data-design diagnosis flags a deeper confound**:
  1. **Topic leakage**: same 23 topics × 9 emotions × 14 stories = 2880. The story-gen LLM produces near-identical scenes when given the same topic with different emotion labels — same protagonist name (Elias appears in both surprise-{0,5,10}-0 and afraid-{0,5,10}-0), same setting, same event, only minor variation in body-language descriptors.
  2. **Forced register collapse**: `inputs/emotions/concept_prompt.txt` mandates "NEVER use the emotion word, convey only through body language, dialogue, physical sensations". Top-20 content words for surprise/afraid/awestruck overlap on ~14/20 entries (felt, eyes, against, throat, breath, stared, stood, room, hands, voice, cold). The "indirect-only" rule forces all high-arousal emotions into the same vocabulary.
  3. **Neutral baseline structural mismatch**: `inputs/emotions/neutral_prompt.txt` generates *Person↔AI Q&A dialogues*, not narratives. Top _neutral words: person(501), list, name, python, text, data, return, legal, email. So every "vs neutral" comparison run in this analysis is actually **narrative prose vs Q&A dialogue**, not "emotional vs unemotional". Every "surprise-specific" feature found may be a narrative-prose feature that doesn't fire on Q&A — including feat 94071 (surprisal-modulated content) and feat 46443 (post-impact recalibration).
  4. **Emotion-word leakage**: despite the prompt rule, "fear" tokens appear in afraid stories at 13.7/10k and in surprise at 6.4/10k; "surprise" tokens appear in awestruck at 8.7/10k (more than in surprise itself, 1.5/10k). Indirect-only rule is poorly enforced.
- **Implication**: every interpretive claim about "surprise feature X" or "epistemic feature Y" in this analysis is contaminated by these confounds. The story-level structure conflates concepts via shared topics; mean-diff vs neutral conflates emotional-prose vs dialogue; the conflate-test only checks afraid (not awestruck or other arousal-matched concepts). The findings so far (L35 feat 94071, L30 feat 46443) survive *as features the SAE represents*, but the labels assigned to them ("epistemic", "surprise_affective") are not trustworthy.
- **Better approaches** (ranked by cost/value):
  1. **[low cost] Replace neutral baseline** with neutral *narratives* (same prompt template + same 23 topics, but flat-affect content) instead of Q&A dialogues. Re-run Phase 2/3/4 on the same surprise stories vs the new neutral. Direct test of how much "emotional vs neutral" signal collapses to "narrative vs dialogue".
  2. **[low cost] Topic-paired contrast** — compute per-topic deltas `mean_F[surprise-T] − mean_F[neutral-T]` and aggregate across topics. Cancels topic-specific lexical content.
  3. **[low cost] Multi-class one-vs-rest specificity** — replace `mean_surprise / (mean_surprise + mean_afraid)` with `mean_surprise / mean_top3_other_emotions`. Exposes surprise+awestruck conflation that single-control specificity misses.
  4. **[medium] Sentence-level aggregation** — separate body-language description sentences from interpretation/reaction sentences via LLM judgment, aggregate only on the latter.
  5. **[medium] Minimal-pair stories** — generate same-prefix stories with diverging final 2 sentences (e.g., "...gasped in wonder" vs "...stepped back in dread"). Read SAE only on the divergence.
  6. **[high] External corpora** — replace LLM-generated stories with human-authored emotion-labeled text (GoEmotions, EmoBank, literary excerpts). Removes the "label and content from the same model" circularity.
- **Next**: take approach (1) — regenerate neutral narratives with the same prompt template + same 23 topics but explicit "flat-affect, no emotional content" instructions. Then re-run Phase 2 on just the new neutral stories (surprise/afraid/etc. activations are unchanged), and re-run Phases 3+4 with the fixed baseline. Goal: identify which of the existing "surprise-specific" features survive a fair (narrative-prose) baseline. Approach (3) — multi-class specificity — is essentially free and worth running in parallel.
